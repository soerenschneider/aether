...
func toEntry(event *ical.Component) Entry {
	...
	if !entry.Start.IsZero() && !entry.End.IsZero() {
		...
		entry.Formatted = fmt.Sprintf("%s – %s", entry.start, entry.end)
	}
...
func getFormattedDate(start, end time.Time) string {
	...
	if pkg.AtSameDay(start, end) {
		if pkg.IsToday(start, now) {
			...
			e := end.Format("15:04")
			return fmt.Sprintf("Today, %s – %s", s, e)
		}
		...
		if pkg.IsTomorrow(start) {
			...
			s := start.Format("15:04")
			e := end.Format("15:04")
			return fmt.Sprintf("Tomorrow, %s – %s", s, e)
		}

		if isWholeDay {
			s := start.Format("02.01.")
			return fmt.Sprintf("%s, %s", start.Weekday().String()[:3], s)
		}

		s := start.Format("02.01. 15:04")
		e := end.Format("15:04")
		return fmt.Sprintf("%s, %s – %s", start.Weekday().String()[:3], s, e)
	}
	...
	if int(dur.Hours()/24) > 0 {
		...
		return fmt.Sprintf("%s – %s (%d days)", formatDate(start, isWholeDay), formatDate(end, isWholeDay), days)
	}

	return fmt.Sprintf("%s – %s", formatDate(start, isWholeDay), formatDate(end, isWholeDay))
}
